    ...
    const PVS_DATA = {
      europoshta: {
        'Минск': [
          'ОПС №119: пр-т Дзержинского, 119',
          'ОПС №11: ул. Петра Мстиславца, 11 (Dana Mall)',
          'ОПС №29: ул. Притыцкого, 29',
          'ОПС №9: пр-т Победителей, 9 (Galleria)',
          'ОПС №159: ул. Лобанка, 94',
          'ОПС №76: ул. Романовская Слобода, 13'
        ],
        'Гомель': [
          'ОПС №97: ул. Советская, 97',
          'ОПС №3: пр-т Ленина, 3',
          'ОПС №22: Речицкий проспект, 5А'
        ],
        'Брест': [
          'ОПС №273: ул. Московская, 273А',
          'ОПС №16: пр-т Машерова, 16',
          'ОПС №4: ул. Гоголя, 65'
        ],
        'Гродно': [
          'ОПС №81: ул. Космонавтов, 81',
          'ОПС №91: ул. Советских Пограничников, 91',
          'ОПС №5: пр-т Янки Купалы, 87'
        ],
        'Витебск': [
          'ОПС №26: ул. Ленина, 26А',
          'ОПС №81: пр-т Фрунзе, 81',
          'ОПС №10: ул. Богатырева, 15'
        ],
        'Могилев': [
          'ОПС №57: ул. Первомайская, 57',
          'ОПС №30: пр-т Пушкинский, 30',
          'ОПС №12: ул. Якубовского, 44'
        ],
        'default': [
          'Центральное отделение (Европочта)',
          'Второе отделение (Европочта)'
        ]
      },
      sdek: {
        'Минск': [
          'МСК1: ул. Притыцкого, 79',
          'МСК2: ул. Левкова, 9',
          'МСК3: пр-т Независимости, 95',
          'МСК4: ул. Сурганова, 50'
        ],
        'Гомель': [
          'ГМЛ1: ул. Кирова, 55',
          'ГМЛ2: Речицкий проспект, 80'
        ],
        'Брест': [
          'БРТ1: бульвар Космонавтов, 40',
          'БРТ2: ул. Карьерная, 12'
        ],
        'Гродно': [
          'ГРД1: ул. Карла Маркса, 29',
          'ГРД2: ул. Горького, 91'
        ],
        'Витебск': [
          'ВТБ1: пр-т Строителей, 11А',
          'ВТБ2: ул. Ленина, 54'
        ],
        'Могилев': [
          'МГЛ1: ул. Первомайская, 12',
          'МГЛ2: ул. Гагарина, 83'
        ],
        'default': [
          'Центральный пункт выдачи СДЭК'
        ]
      },
      belpochta: {
        'Минск': [
          'ПО-100: пр-т Независимости, 10',
          'ПО-1: ул. Главная, 25',
          'ПО-50: ул. Кирова, 8'
        ],
        'default': [
...
const MAX_REQUESTS_TRUSTED = 100;
    ...
    async function init() {
      ...
      try {
        ...
        if (avatarDiv) {
          avatarDiv.innerHTML = userAvatarUrl ? `<img src="${userAvatarUrl}">` : (userName || 'Гость').charAt(0).toUpperCase();
        }
...
document.querySelectorAll('.buyNowBtn').forEach(btn => {
  ...
  try {
    ...
    async function renderCalculator() {
        ...
        vacationBanner = `
          <div class="mb-4 p-4 rounded-2xl border border-amber-500/30 bg-amber-500/10 text-amber-300 text-sm flex gap-3 items-center">
            ...
            <div class="text-left">
              ...
              <p class="text-xs text-white/70 mt-0.5">Сроки доставки увеличены на ${days} дн. Расчёты в калькуляторе учитывают это время.</p>
            </div>
          </div>
        `;
      }
      ...
      if (!window.calcCountry) {
        return `
          <div class="glass-card page-enter">
            <h2 class="text-xl font-bold mb-4 text-center"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg></span> Калькулятор стоимости</h2>
            <p class="text-white/60 text-sm text-center mb-6">Выберите страну, откуда будет отправлен товар. От этого зависят тарифы и сроки.</p>
            ${vacationBanner}
            ${limitMessage}
            <div class="grid grid-cols-1 gap-4">
              ...
              <div class="glass-card p-4 border border-white/10 hover:border-cyan-500/50 cursor-pointer transition flex items-center gap-4" onclick="selectCalcCountry('CN')">
                ...
                <div class="flex-1">
                  <h3 class="font-bold text-white text-base">Китай (Poizon, Taobao, 1688)</h3>
                  <p class="text-xs text-white/50 mt-0.5">Огромный выбор, лучшие цены на оригиналы</p>
                  <div class="flex gap-4 mt-2 text-xs">
                    <span class="text-cyan-400 font-semibold"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg></span> $10 / кг</span>
                    <span class="text-white/60"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></span> 10-15 дней</span>
                  </div>
                </div>
              </div>
              <!-- Poland/EU -->
              <div class="glass-card p-4 border border-white/10 hover:border-cyan-500/50 cursor-pointer transition flex items-center gap-4" onclick="selectCalcCountry('PL')">
                <div class="text-4xl">🇵🇱</div>
                <div class="flex-1">
                  <h3 class="font-bold text-white text-base">Польша / ЕС (Zalando, ASOS, Farfetch)</h3>
                  <p class="text-xs text-white/50 mt-0.5">100% оригинальные европейские коллекции</p>
                  <div class="flex gap-4 mt-2 text-xs">
                    <span class="text-cyan-400 font-semibold"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg></span> $15 / кг</span>
                    <span class="text-white/60"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></span> 14-20 дней</span>
                  </div>
                </div>
              </div>
              <!-- Russia -->
              <div class="glass-card p-4 border border-white/10 hover:border-cyan-500/50 cursor-pointer transition flex items-center gap-4" onclick="selectCalcCountry('RU')">
                <div class="text-4xl">🇷🇺</div>
                <div class="flex-1">
                  <h3 class="font-bold text-white text-base">Россия (WB, Lamoda, Ozon)</h3>
                  <p class="text-xs text-white/50 mt-0.5">Сверхбыстрая доставка, без пошлин ЕАЭС</p>
                  <div class="flex gap-4 mt-2 text-xs">
                    <span class="text-cyan-400 font-semibold"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg></span> $5 / кг</span>
                    <span class="text-white/60"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></span> 3-5 дней</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      const countryLabel = window.calcCountry === 'CN' ? '🇨🇳 Китай' : window.calcCountry === 'PL' ? '🇵🇱 Польша / ЕС' : '🇷🇺 Россия';
      const defaultCurrency = window.calcCountry === 'CN' ? 'CNY' : window.calcCountry === 'RU' ? 'RUB' : 'EUR';

      return `
        <div class="glass-card page-enter">
          <div class="flex justify-between items-center mb-4">
            <h2 class="text-lg font-bold"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg></span> Расчёт стоимости</h2>
            <button class="text-xs text-cyan-400 bg-white/5 px-3 py-1.5 rounded-full hover:bg-white/10 transition" onclick="window.calcCountry=null; renderCurrentScreen()">
              ${countryLabel} <span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/></svg></span> Сменить
            </button>
          </div>
          ${vacationBanner}
          ${limitMessage}
          ...
          <div data-calc-mode-pane="link manual">
            ...
            <div class="flex flex-wrap gap-2 mt-1 mb-1 items-stretch">
              <div class="flex-1 min-w-[200px] flex gap-1 bg-white/5 border border-white/30 rounded-xl overflow-hidden px-1.5 py-1">
                ...
                <button type="button" id="calcPasteBtn" class="bg-white/10 hover:bg-white/20 border border-white/10 rounded-lg px-3.5 transition flex items-center justify-center text-cyan-400 gap-1.5 text-xs font-bold" title="Вставить из буфера">
          ...
          <!-- Price & Currency — single row -->
          <div class="mb-3">
            <label class="text-white/70 text-sm block mb-1">Цена с валютой</label>
            <div class="flex gap-2">
              <div class="flex-1 min-w-0">
                <input type="number" id="calcPrice" class="btn-secondary w-full p-3 rounded-xl border border-white/30 text-sm" placeholder="Укажите цену товара">
              </div>
              <div class="w-[110px] flex-shrink-0">
                <select id="calcCurrency" class="btn-secondary w-full p-3 rounded-xl border border-white/30 text-sm" style="padding: 14px 10px !important;">
                  ...
                  <option value="RUB" ${defaultCurrency === 'RUB' ? 'selected' : ''}>RUB (₽)</option>
          ...
          <p id="calcCategoryHint" class="text-cyan-400 text-[10px] hidden mb-3 font-semibold ml-1">💡 Обычно весит ~1.2 кг</p>
          <div class="flex justify-between items-center mt-2 mb-0.5">
            ...
            <button type="button" class="text-xs text-cyan-400 flex items-center gap-1 bg-white/5 px-2.5 py-1 rounded-lg hover:bg-white/10 transition" onclick="showSizeTablesModal()">
              ${ix('compare', { size: '12px' })} Таблица размеров
            </button>
          </div>
          <input type="text" id="calcSize" class="btn-secondary w-full p-3 rounded-xl border border-white/30 mb-3" placeholder="Размер (напр. 42, M, L)">
          ...
          <div class="border border-cyan-500/20 bg-cyan-500/5 p-3 rounded-xl mb-3 text-left">
            <p class="text-cyan-400 text-xs font-bold mb-1 flex items-center gap-1"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M21 21H3L21 3v18z"/><path d="M15 15l2-2"/><path d="M11 19l2-2"/><path d="M13 13l2-2"/></svg></span> Ввод замеров для ИИ-подбора размера (Рекомендуется)</p>
            <p class="text-white/50 text-[10px] mb-2">Наш партнер подберет размер идеально на основе этих данных</p>
            ...
            <div id="calcAiSizeAdvisorBanner" class="mt-3 p-3 rounded-xl border border-violet-500/30 bg-violet-500/10 text-xs text-white/80 hidden cursor-pointer">
              <p class="text-violet-400 font-bold flex items-center gap-1">
                <span>🔮 ИИ-Советник по размеру</span>
              </p>
              <p class="mt-1 font-medium" id="calcAiSizeAdvisorText">Рекомендуем вам размер ... на основе ваших замеров.</p>
              <p class="text-white/40 text-[9px] mt-1 leading-normal">✨ Нажмите, чтобы применить рекомендованный размер</p>
            </div>
          </div>
  ...
  if (calcBtn) calcBtn.onclick = async () => {
      ...
      const comparisonHtml = `
        <div class="mt-3 p-3.5 rounded-xl border border-yellow-500/20 bg-yellow-500/5 text-xs space-y-2">
          <p class="text-yellow-400 font-bold flex items-center gap-1.5">
            ...
            <span>Выгода заказа в ICE LOGIX</span>
          </p>
          <div class="grid grid-cols-2 gap-2 text-[11px] leading-relaxed">
            <div class="bg-white/5 p-2 rounded-lg">
              <span class="text-white/40 block">В ТЦ Минска:</span>
            ...
            <div class="bg-white/5 p-2 rounded-lg border border-cyan-500/20">
              <span class="text-cyan-400/60 block font-bold">Наша цена:</span>
          ...
          <p class="text-white/80 font-medium text-[11px] text-center pt-1 border-t border-white/5">
            🔥 Ваша чистая экономия: <strong class="text-green-400 font-bold font-mono text-xs">${savings.toFixed(0)} BYN (${savingsPercent}%)</strong>!
          </p>
        </div>
      `;
    ...
    if (shareBtn) {
      shareBtn.onclick = () => {
        const title = document.getElementById('calcTitle')?.value || category || 'Товар';
        const msg = `🔥 *${title}*
Цена с доставкой: *${result.total_byn.toFixed(2)} BYN*
...
Считал через ICE LOGIX! Заказываем вместе?`;
  ...
  // Применяем предустановленный режим (если был переход с Главной по "Поиск по фото")
  if (window.calcPreselectMode) {
    setCalcMode(window.calcPreselectMode);
    window.calcPreselectMode = null;
  } else {
    setCalcMode('link'); // по умолчанию
  }
}
    ...
    async function renderNewOrder() {
      ...
      if (!window.orderCountry) {
        ...
        return `
          <div class="glass-card page-enter">
            <h2 class="text-xl font-bold mb-4 text-center"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg></span> Выберите страну доставки</h2>
            <p class="text-white/60 text-sm text-center mb-6">От страны зависят тарифы на доставку, сроки и таможенные пошлины.</p>
            ${vacationBanner}
            ...
            <div class="grid grid-cols-1 gap-4">
              ...
              <div class="glass-card p-4 border border-white/10 hover:border-cyan-500/50 cursor-pointer transition flex items-center gap-4" onclick="selectOrderCountry('CN')">
                ...
                <div class="flex-1">
                  ...
                  <p class="text-xs text-white/50 mt-0.5">Огромный выбор, лучшие цены на оригиналы</p>
                  <div class="flex gap-4 mt-2 text-xs">
                    <span class="text-cyan-400 font-semibold">⚡ $10 / кг</span>
                    <span class="text-white/60"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></span> 10-15 дней</span>
                  </div>
                </div>
              </div>
              ...
              <div class="glass-card p-4 border border-white/10 hover:border-cyan-500/50 cursor-pointer transition flex items-center gap-4" onclick="selectOrderCountry('PL')">
                ...
                <div class="flex-1">
                  ...
                  <p class="text-xs text-white/50 mt-0.5">100% оригинальные европейские коллекции</p>
                  <div class="flex gap-4 mt-2 text-xs">
                    <span class="text-cyan-400 font-semibold">⚡ $15 / кг</span>
                    <span class="text-white/60"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></span> 14-20 дней</span>
                  </div>
                </div>
              </div>
              ...
              <div class="glass-card p-4 border border-white/10 hover:border-cyan-500/50 cursor-pointer transition flex items-center gap-4" onclick="selectOrderCountry('RU')">
                ...
                <div class="flex-1">
                  ...
                  <p class="text-xs text-white/50 mt-0.5">Сверхбыстрая доставка, без пошлин ЕАЭС</p>
                  <div class="flex gap-4 mt-2 text-xs">
                    <span class="text-cyan-400 font-semibold">⚡ $5 / кг</span>
                    <span class="text-white/60"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></span> 3-5 дней</span>
                  </div>
                </div>
              </div>
      ...
      if (!window.orderAddMode) {
        ...
        const checkoutOptionsAndTotals = totalSum > 0 ? `
          ...
          <div class="bg-white/5 p-4 rounded-xl mb-4 space-y-3">
            ...
            <div>
              <label class="text-white/50 text-[10px] block mb-1">Служба доставки</label>
              <select id="pvsMethodSelect" class="w-full p-3 rounded-xl border border-white/20 text-sm bg-slate-900 text-white">
                <option value="none" ${(!window.tempOrder.pvs || window.tempOrder.pvs.method === 'none') ? 'selected' : ''}>Самовывоз / Почта (Без дополнительной платы)</option>
                <option value="europoshta" ${(window.tempOrder.pvs?.method === 'europoshta') ? 'selected' : ''}>Европочта (до отделения) — 5.00 BYN</option>
                <option value="belpochta" ${(window.tempOrder.pvs?.method === 'belpochta') ? 'selected' : ''}>Белпочта (до отделения) — 5.00 BYN</option>
                <option value="sdek" ${(window.tempOrder.pvs?.method === 'sdek') ? 'selected' : ''}>СДЭК (до отделения) — 8.00 BYN</option>
              </select>
            </div>
      ...
      return `
        <div class="glass-card page-enter">
          <div class="flex justify-between items-center mb-4">
            <h2 class="text-lg font-bold"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></span> Добавить новый товар</h2>
            <button class="text-xs text-white/50 hover:text-white" onclick="stopOrderAddMode()">
              <span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M19 12H5"/><path d="m12 19-7-7 7-7"/></svg></span> Назад к списку
            </button>
          </div>
          
          <!-- Step 3: Dual-Section Mode Selector -->
          <div class="mb-4">
            <p class="text-white/60 text-xs mb-2">Ввод данных (если вы сами нашли товар):</p>
            <div class="grid grid-cols-2 gap-2 mb-4">
              <button type="button" class="order-mode-btn ${subMode === 'manual' ? 'btn-primary' : 'btn-secondary'} transition text-xs font-bold py-2.5" onclick="setOrderAddSubMode('manual')"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></span> Вручную</button>
              <button type="button" class="order-mode-btn ${subMode === 'link' ? 'btn-primary' : 'btn-secondary'} transition text-xs font-bold py-2.5" onclick="setOrderAddSubMode('link')"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg></span> По ссылке / Скриншоту</button>
            </div>
            
            <p class="text-white/60 text-xs mb-2">Поиск товара (если хотите найти через ИИ):</p>
            <div class="grid grid-cols-2 gap-2">
              <button type="button" class="order-mode-btn ${subMode === 'photo' ? 'btn-primary' : 'btn-secondary'} transition text-xs font-bold py-2.5" onclick="setOrderAddSubMode('photo')"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/></svg></span> По фото</button>
              <button type="button" class="order-mode-btn ${subMode === 'text' ? 'btn-primary' : 'btn-secondary'} transition text-xs font-bold py-2.5" onclick="setOrderAddSubMode('text')"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg></span> По описанию</button>
            </div>
          </div>
          ...
          <div id="paneOrderLink" class="mode-pane ${subMode === 'link' ? '' : 'hidden'} mb-4">
            <label class="text-white/70 text-sm block mb-1">Ссылка на товар</label>
          ...
          <div id="paneOrderPhoto" class="mode-pane ${subMode === 'photo' ? '' : 'hidden'} mb-4">
            ...
            <label class="text-white/70 text-xs mt-2 block">Описание (опционально) — улучшит поиск</label>
            <input id="orderPhotoHint" type="text" placeholder="Напр. Calvin Klein zip hoodie серый M" class="btn-secondary w-full p-3 rounded-xl border border-white/30 text-sm">
          ...
          <div id="paneOrderText" class="mode-pane ${subMode === 'text' ? '' : 'hidden'} mb-4">
            ...
            <p class="text-white/40 text-xs mt-1">ИИ распознает бренд, модель и категорию автоматически.</p>
          ...
          <div id="orderManualForm" class="mt-4 border-t border-white/10 pt-4">
            ...
            <div class="mb-3">
              ...
              <div id="orderManualPhotoUploadZone" class="screenshot-upload-zone p-3 border border-dashed border-white/30 rounded-xl text-center cursor-pointer hover:bg-white/5 transition">
                <span class="text-sm text-white/70"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></span> Загрузить скриншот / фото товара</span>
            ...
            <div class="bg-white/5 p-3 rounded-2xl mb-3">
              ...
              <div class="mb-2">
                ...
                <input type="text" id="orderBrand" class="btn-secondary w-full p-3 rounded-xl border border-white/30 text-sm" placeholder="Например: Nike, Adidas, Stone Island">
              </div>
              <div class="mb-2">
                <label class="text-white/50 text-xs block mb-0.5">Модель</label>
                <input type="text" id="orderModel" class="btn-secondary w-full p-3 rounded-xl border border-white/30 text-sm" placeholder="Например: Air Force 1, Yeezy 350">
              </div>
              <div>
                <label class="text-white/50 text-xs block mb-0.5">Особенности</label>
                <input type="text" id="orderFeatures" class="btn-secondary w-full p-3 rounded-xl border border-white/30 text-sm" placeholder="Например: светоотражающие шнурки, лимитированная серия">
              </div>
            </div>
            
            <!-- Description Group -->
            <div class="bg-white/5 p-3 rounded-2xl mb-3">
              <p class="text-white/60 text-xs mb-2">Описание и Характеристики:</p>
              
              <!-- Gender Chips selection -->
              <div class="mb-3">
                <label class="text-white/50 text-xs block mb-1">Пол</label>
                <div class="flex gap-2" id="orderGenderSelector">
                  <button type="button" data-gender="male" class="filter-chip flex-1 py-2 text-center text-xs border border-white/10 hover:bg-white/5 transition" onclick="selectOrderGender('male')"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></span> Мужской</button>
                  <button type="button" data-gender="female" class="filter-chip flex-1 py-2 text-center text-xs border border-white/10 hover:bg-white/5 transition" onclick="selectOrderGender('female')"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></span> Женский</button>
                  <button type="button" data-gender="unisex" class="filter-chip flex-1 py-2 text-center text-xs border border-white/10 hover:bg-white/5 transition" onclick="selectOrderGender('unisex')"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></span> Унисекс</button>
              ...
              <div class="mb-3">
                ...
                <p id="orderCategoryHint" class="text-cyan-400 text-[10px] hidden mb-2 font-semibold ml-1">💡 Обычно весит ~1.2 кг</p>
              </div>
              ...
              <div class="grid grid-cols-2 gap-2 mb-3">
                ...
                <div>
                  <div class="flex justify-between items-center mb-0.5">
                    <label class="text-white/50 text-xs block">Размер</label>
                  ...
                  <input type="text" id="orderSize" class="btn-secondary w-full p-3 rounded-xl border border-white/30 text-sm" placeholder="Напр. US 9, EU 42.5">
                </div>
              </div>
              ...
              <div class="border border-cyan-500/20 bg-cyan-500/5 p-3 rounded-xl">
                ...
                <div class="grid grid-cols-3 gap-2">
                  ...
                  <div>
                    <label class="text-white/40 text-[9px] block">Стелька / Обхват (см)</label>
                ...
                <div id="sanctionBanner" class="mt-3 p-3 rounded-xl border border-orange-500/30 bg-orange-500/10 text-xs text-white/90 hidden">
                  <p class="text-orange-400 font-bold flex items-center gap-1">
                    <span>⚠️ Внимание: Санкционный бренд</span>
                  </p>
                  <p class="mt-1 font-medium text-white/80">Товары этого бренда могут быть задержаны таможней ЕС. Наш менеджер проверит возможность транзита после оформления заказа.</p>
                </div>
                ...
                <div id="aiSizeAdvisorBanner" class="mt-3 p-3 rounded-xl border border-violet-500/30 bg-violet-500/10 text-xs text-white/80 hidden cursor-pointer">
                  ...
                  <p class="mt-1 font-medium" id="aiSizeAdvisorText">Рекомендуем вам размер ... на основе ваших замеров.</p>
                  <p class="text-white/40 text-[9px] mt-1 leading-normal">✨ Нажмите, чтобы применить рекомендованный размер</p>
                </div>
              </div>
            </div>
            ...
            <div class="mb-4">
              ...
              <div class="flex items-center gap-3 mt-1">
                ...
                <span id="orderWeightVal" class="text-white/80 bg-white/10 px-3 py-1 rounded-full whitespace-nowrap text-xs font-bold">1.0 кг</span>
              </div>
            </div>
            ...
            <div class="flex flex-col gap-2 mt-4">
              <button id="orderSaveItemBtn" class="btn-primary w-full py-3.5 rounded-xl font-bold transition flex items-center justify-center gap-2">
                <span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg></span> Добавить товар в заказ
              </button>
        ...
        if (applyPromoBtn) {
          applyPromoBtn.onclick = async () => {
            ...
            try {
              ...
              promoMessage.innerHTML = `Промокод применён! Скидка: ${discountAmount.toFixed(2)} <span class="brand-flake" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="12" y1="2" x2="12" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/><line x1="4.9" y1="4.9" x2="19.1" y2="19.1"/><line x1="19.1" y1="4.9" x2="4.9" y2="19.1"/><polyline points="8 5 12 2 16 5"/><polyline points="8 19 12 22 16 19"/><polyline points="5 8 2 12 5 16"/><polyline points="19 8 22 12 19 16"/></svg></span>`;
              promoMessage.classList.remove('hidden');
    ...
    <div class="p-4 rounded-2xl mb-4" style="background: linear-gradient(135deg, rgba(52,211,153,0.15), rgba(16,185,129,0.08)); border: 1px solid rgba(52,211,153,0.25);">
      ...
      <div class="grid grid-cols-2 gap-3 mb-3">
        ...
        <div class="p-3 rounded-xl text-center" style="background: var(--glass-bg);">
          ...
          <p class="text-[10px] uppercase tracking-wider flex items-center justify-center gap-1" style="color: var(--text-muted);">Бонусы <span class="brand-flake" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="12" y1="2" x2="12" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/><line x1="4.9" y1="4.9" x2="19.1" y2="19.1"/><line x1="19.1" y1="4.9" x2="4.9" y2="19.1"/><polyline points="8 5 12 2 16 5"/><polyline points="8 19 12 22 16 19"/><polyline points="5 8 2 12 5 16"/><polyline points="19 8 22 12 19 16"/></svg></span></p>
        </div>
      </div>
      ${userReferralCode ? `
  ...
  <div class="mb-5 page-enter" style="animation-delay: 0.15s;">
    ...
    <div class="glass-card p-2">
      <button id="myPersonalDataBtn" class="w-full p-3 rounded-xl flex items-center gap-3 text-left hover:bg-white/5 transition">
        ...
        <div class="flex-1">
          <p class="font-semibold text-white text-sm">Мои данные</p>
          <p class="text-xs" style="color: var(--text-muted);">ФИО, телефон, паспорт и замеры</p>
        </div>
      ...
      <div class="w-full p-3 rounded-xl flex items-center justify-between border-t border-white/5">
        <div class="flex items-center gap-3">
          ...
          <div>
            <p class="font-semibold text-white text-sm">Светлая тема</p>
            <p class="text-xs" style="color: var(--text-muted);">Внешний вид</p>
          </div>
        </div>
          ...
          <div id="statsContainer">
            <div class="space-y-4">
              ...
              <div class="glass-card">
                ...
                <div class="grid grid-cols-2 gap-3">
                  <div class="bg-[var(--ice-primary)]/10 border border-[var(--ice-primary)]/20 rounded-xl p-3 text-center"><p class="text-white/70 text-[10px] uppercase tracking-wider">Заработано</p><p class="text-green-400 text-xl font-bold">${totalEarned} <span class="brand-flake" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="12" y1="2" x2="12" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/><line x1="4.9" y1="4.9" x2="19.1" y2="19.1"/><line x1="19.1" y1="4.9" x2="4.9" y2="19.1"/><polyline points="8 5 12 2 16 5"/><polyline points="8 19 12 22 16 19"/><polyline points="5 8 2 12 5 16"/><polyline points="19 8 22 12 19 16"/></svg></span></p></div>
                  <div class="bg-white/5 border border-white/10 rounded-xl p-3 text-center"><p class="text-white/70 text-[10px] uppercase tracking-wider">Доступно к выводу</p><p class="text-cyan-400 text-xl font-bold">${availableForPayout} <span class="brand-flake" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="12" y1="2" x2="12" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/><line x1="4.9" y1="4.9" x2="19.1" y2="19.1"/><line x1="19.1" y1="4.9" x2="4.9" y2="19.1"/><polyline points="8 5 12 2 16 5"/><polyline points="8 19 12 22 16 19"/><polyline points="5 8 2 12 5 16"/><polyline points="19 8 22 12 19 16"/></svg></span></p></div>
                  <div class="bg-white/5 border border-white/10 rounded-xl p-3 text-center col-span-2"><p class="text-white/70 text-[10px] uppercase tracking-wider">Порог вывода</p><p class="text-white text-lg font-bold">${settings.payout_threshold} <span class="brand-flake" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="12" y1="2" x2="12" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/><line x1="4.9" y1="4.9" x2="19.1" y2="19.1"/><line x1="19.1" y1="4.9" x2="4.9" y2="19.1"/><polyline points="8 5 12 2 16 5"/><polyline points="8 19 12 22 16 19"/><polyline points="5 8 2 12 5 16"/><polyline points="19 8 22 12 19 16"/></svg></span></p></div>
                </div>
              </div>
              <div class="glass-card">
  ...
  btn.onclick = (e) => {
    ...
    async function renderReports() {
      try {
        ...
        try {
          ...
          statsHtml = `<div class="glass-card mb-4 p-3 flex justify-around items-center text-center"><div class="flex-1 border-r border-white/10"><p class="text-white/50 text-[10px] uppercase">Выкуплено</p><p class="text-white font-bold text-lg">${data ? data.length : 0}</p></div><div class="flex-1"><p class="text-white/50 text-[10px] uppercase">Доставлено</p><p class="text-cyan-400 font-bold text-lg">${count || '120+'}</p></div></div>`;
        } catch(e) {}
    ...
    function attachReportsHandlers() {
      ...
      if (addBtn) {
        addBtn.onclick = () => {
          ...
          modal.innerHTML = `
            <div class="glass-card max-w-md w-full max-h-[90vh] overflow-y-auto">
              ...
              <div id="mediaUrlsContainer" class="space-y-2 mb-2">
                <input type="url" class="reportUrlInput btn-secondary w-full p-3 rounded-xl border border-white/30" placeholder="Ссылка на фото/видео (Реальность)">
              </div>
              <button id="addMediaUrlBtn" class="text-cyan-400 text-xs mb-4 font-bold flex items-center gap-1"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></span> Добавить еще фото (например, чек)</button>
          ...
          modal.querySelector('#addMediaUrlBtn').onclick = () => {
            ...
            input.placeholder = 'Еще ссылка на фото/видео';
          ...
          modal.querySelector('#saveReportBtn').onclick = async () => {
            ...
            if (!title || urlInputs.length === 0) { tgUtil.alert('Заполните заголовок и хотя бы одну ссылку на фото/видео'); return; }
      ...
      try {
        ...
        // Если ошибка связана с отсутствием столбца region, fallback на все отзывы
        let finalData = data;
        ...
        const regions = [
          ...
          { id: 'usa', label: 'США 🇺🇸', disabled: true },
          { id: 'uae', label: 'ОАЭ 🇦🇪', disabled: true }
        ];
            ...
            <div class="flex gap-2 overflow-x-auto pb-2 hide-scrollbar" id="reviewsRegionTabs">
              ${regions.map(r => `
                <button class="filter-chip ${window.currentReviewsRegion === r.id ? 'active' : ''} ${r.disabled ? 'opacity-50 cursor-not-allowed' : ''}" 
                        data-region="${r.id}" ${r.disabled ? 'disabled title="Скоро"' : ''}>
                  ${r.label}
            ...
            ${!finalData || finalData.length === 0 ? '<p class="text-white/70">Пока нет отзывов в этой категории</p>' : finalData.map(review => {
              ...
              <div class="review-card glass-card p-4 rounded-2xl mb-4">
                <div class="flex items-center justify-between mb-2">
                  <div>
                    <p class="text-white font-bold flex items-center gap-2">${review.user_name || 'Пользователь'} ${review.is_verified ? '<span class="ix ix-success text-sm" title="Заказ подтвержден"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span>' : ''}</p>
                ...
                <div class="flex gap-2 mt-3 mb-3">
                  ...
                  <div class="flex-1 relative rounded-xl overflow-hidden border border-white/10" style="aspect-ratio: 1/1;">
                    <span class="absolute top-1 left-1 bg-black/60 text-[10px] px-1.5 py-0.5 rounded text-white/90 z-10">Сайт</span>
                    <img src="${siteImg}" class="w-full h-full object-cover cursor-pointer hover:scale-105 transition" onclick="tgUtil.popup('${siteImg}', 'Фото с сайта')">
                  </div>
                  ` : ''}
                  ${hasClientPhotos ? `
                  <div class="flex-1 relative rounded-xl overflow-hidden border border-cyan-500/30" style="aspect-ratio: 1/1;">
                    <span class="absolute top-1 left-1 bg-cyan-500/80 text-[10px] px-1.5 py-0.5 rounded text-white z-10">Реальность</span>
                    <img src="${review.photo_urls[0]}" class="w-full h-full object-cover cursor-pointer hover:scale-105 transition" onclick="tgUtil.popup('${review.photo_urls[0]}', 'Фото клиента')">
    ...
    function attachReviewsHandlers() {
      ...
      if (splitOrderBtn) {
        splitOrderBtn.onclick = () => {
          alert('Функция разделения посылки формирует два заказа с разными трек-номерами. Вы сможете указать второго получателя на следующем этапе!');
        }
      }
        ...
        leaveBtn.onclick = async () => {
          ...
          modal.innerHTML = `
            <div class="glass-card max-w-md w-full">
              ...
              <select id="reviewRegion" class="btn-secondary w-full p-3 rounded-xl border border-white/30 mb-3">
                <option value="">Укажите регион площадки</option>
                <option value="china">Китай (Poizon, 1688...)</option>
                <option value="europe">Европа (ASOS, Zalando...)</option>
                <option value="russia">Россия (Wildberries, Ozon...)</option>
              </select>
              ...
              <div class="mb-3">
                <label class="block text-white/70 text-sm mb-1">Ваше фото "Вживую" (Бонус 5 BYN после проверки)</label>
          ...
          modal.querySelector('#submitReviewBtn').onclick = async () => {
            ...
            try {
              ...
              modal.remove();
            } catch (err) { tgUtil.alert('Ошибка: ' + err.message); submitBtn.disabled = false; submitBtn.innerText = 'Отправить'; }
          };
        };
      }
    }
...
if (createBtn) {
  createBtn.onclick = async () => {
    if (!window.tempOrder || !userId) { alert('Ошибка: заполните все поля'); return; }
    try {
      const { data, error } = await supabaseClient.from('orders').insert({
        user_id: userId,
        ...
        weight_estimated: window.tempOrder.weight || 1,
        price_original: window.tempOrder.price || 0,
        price_byn: (window.tempOrder.price || 0) * 0.45 * 1.20,
        delivery_cost_estimated: (window.tempOrder.weight || 1) * 12,
        prepayment_amount: (window.tempOrder.total - window.tempOrder.discountAmount) * 0.75,
        payment_method: document.getElementById('orderPaymentMethod')?.value || 'erip',
        delivery_method: document.getElementById('orderDeliveryMethod')?.value || 'belpost',
        cny_rate: window.tempOrder.cny_rate || 0.45,
        status: 'pending',
        discount_applied: window.tempOrder.discountAmount || 0,
        promo_code: window.tempOrder.appliedPromo?.code || null,
        extra_services: {
          vacuum: document.getElementById('orderVacuum')?.checked || false,
          bubble_wrap: document.getElementById('orderBubbleWrap')?.checked || false,
          keep_box: document.getElementById('orderKeepBox')?.checked || false
        }
      }).select();
      if (error) throw error;

      alert(`Заказ создан! Номер: ${data[0].id}. Сумма предоплаты: ${((window.tempOrder.total - window.tempOrder.discountAmount) * 0.75).toFixed(2)} ❄️`);
      ...
      if (!userLimits.isTrusted) {
        await supabaseClient.from('users').update({ is_trusted: true }).eq('user_id', userId);
        userLimits.isTrusted = true;
        alert('🎉 Поздравляем с первым заказом! Лимит на расчёты увеличен до 100 в день.');
      }
        ...
        <div class="p-3 bg-white/5 border border-white/10 rounded-xl space-y-2 mb-2 text-left page-enter font-sans">
          ...
          <div class="text-xs text-white leading-normal">
            ...
            <div class="mt-3 flex gap-2">
              <button class="flex-1 bg-white/20 hover:bg-white/30 py-2 rounded-lg text-sm text-white" onclick="window.open('https://t.me/icelogix_bot?text=Вопрос по заказу ${order.id.slice(0,8)}', '_blank')">💬 Поддержка</button>
              <button class="flex-1 bg-blue-500/50 hover:bg-blue-500/70 py-2 rounded-lg text-sm text-white" onclick="downloadCustomsInvoice('${order.id}')">📄 PDF Инвойс</button>
            </div>
          </div>
          ${isPending ? `
...
function attachAdmin2FAHandlers() {
  ...
  async function checkPin() {
    ...
    } catch(e) { tgUtil.alert('Ошибка ' + e.message); currentPin = ''; updateDots(); }
  }
...
async function sendNotification(userId, message, orderId = null) {
  try {
    ...
    window.renderCatalogHub = async () => {
  return `
    <button class="global-back-btn" onclick="switchTab('home')"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="19" y1="12" x2="5" y2="12"/><polyline points="12 19 5 12 12 5"/></svg></span> Назад</button>
    <div class="glass-card mb-5 page-enter">
      <h2 class="text-xl font-bold text-white mb-2"><span class="ix ix-accent"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 3h7v7H3z"/><path d="M14 3h7v7h-7z"/><path d="M14 14h7v7h-7z"/><path d="M3 14h7v7H3z"/></svg></span> Каталог и Подборки</h2>
      <p class="text-white/50 text-sm mb-4">Выберите категорию для просмотра товаров или ознакомьтесь с нашими гайдами.</p>
      
      <div class="grid grid-cols-2 gap-3 mb-6">
        <div class="bg-gradient-to-br from-blue-500/20 to-purple-500/20 p-4 rounded-xl border border-white/10 cursor-pointer hover:border-blue-400 transition" onclick="switchTab('catalogs', 'productsCatalog')">
          <span class="text-3xl mb-2 block">👟</span>
          <h3 class="text-white font-bold text-sm">Все товары</h3>
          <p class="text-white/50 text-[10px]">Витрина кроссовок и одежды</p>
        </div>
        <div class="bg-gradient-to-br from-emerald-500/20 to-teal-500/20 p-4 rounded-xl border border-white/10 cursor-pointer hover:border-emerald-400 transition" onclick="glassToast('Категория скоро появится!', {kind:'info'})">
          <span class="text-3xl mb-2 block">🎒</span>
          <h3 class="text-white font-bold text-sm">Аксессуары</h3>
          <p class="text-white/50 text-[10px]">Сумки, очки, украшения</p>
        </div>
        <div class="bg-gradient-to-br from-amber-500/20 to-orange-500/20 p-4 rounded-xl border border-white/10 cursor-pointer hover:border-amber-400 transition" onclick="switchTab('catalogs', 'marketplaces')">
          <span class="text-3xl mb-2 block">🛍️</span>
          <h3 class="text-white font-bold text-sm">Площадки</h3>
          <p class="text-white/50 text-[10px]">Poizon, Taobao, 1688</p>
        </div>
        <div class="bg-gradient-to-br from-rose-500/20 to-pink-500/20 p-4 rounded-xl border border-white/10 cursor-pointer hover:border-rose-400 transition" onclick="switchTab('catalogs', 'sizeGuides')">
          <span class="text-3xl mb-2 block">📏</span>
          <h3 class="text-white font-bold text-sm">Размеры</h3>
          <p class="text-white/50 text-[10px]">Гайд по размерным сеткам</p>
        </div>
      </div>
    </div>
    ${renderFooter()}
...
window.renderSizeGuides = async () => {
  return `
    <button class="global-back-btn" onclick="switchTab('catalogs', 'catalogHub')"><span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="19" y1="12" x2="5" y2="12"/><polyline points="12 19 5 12 12 5"/></svg></span> Назад в Каталог</button>
    <div class="glass-card mb-5 page-enter">
...
async function renderAdminOrdersList() {
  try {
    ...
    const from = (adminOrdersPage - 1) * 30; // Увеличим пагинацию для канбана
    const to = from + 29;
    const { data: orders, count, error } = await query.range(from, to);
    if (error) throw error;
  ...
  try {
    ...
    if (userId) {
      try {
        const { data: userOrders } = await supabaseClient.from('orders')
          .select('id')
          .eq('user_id', userId)
          .in('status', ['paid', 'bought', 'on_sklad_cn', 'in_transit', 'in_belarus', 'delivered'])
          .limit(1);
        
        const { data: userTx } = await supabaseClient.from('transactions')
          .select('id')
          .eq('user_id', userId)
          .limit(1);

        if ((userOrders && userOrders.length > 0) || (userTx && userTx.length > 0)) {
          isVipUnlocked = true;
        }
      } catch (e) {
        console.error('Error checking VIP lock status:', e);
      }
    }
    ...
    return `
      ...
      ${isVipUnlocked ? `
        ...
        <div class="glass-card mb-5 overflow-hidden relative border border-green-500/20 bg-gradient-to-br from-slate-900 via-emerald-950/10 to-slate-900 p-4 page-enter cursor-pointer hover:scale-[1.01] transition-all" onclick="window.showVipSectionModal()">
      ...
      ` : `
        ...
        <div class="glass-card mb-5 overflow-hidden relative border border-yellow-500/20 bg-gradient-to-br from-slate-900 via-amber-950/10 to-slate-900 p-4 page-enter cursor-pointer hover:scale-[1.01] transition-all" onclick="tgUtil.alert('Секретный VIP-раздел заблокирован! Оформите и оплатите хотя бы один заказ, чтобы разблокировать доступ к уникальным скидкам до 70%!')">
  ...
  try {
    ...
    return `
      <div class="space-y-4">
        ...
        <!-- Таможенный калькулятор 2.0 -->
        <div class="bg-white/10 backdrop-blur-md rounded-2xl p-4">
          <h3 class="text-white font-bold mb-3">⚖️ Таможенный лимит (РБ)</h3>
          <p class="text-white/50 text-xs mb-2">Настройте беспошлинный лимит (в евро) и процент пошлины за превышение для умного калькулятора.</p>
          <label class="text-white/70 text-sm">Беспошлинный лимит (€)</label>
          <input type="number" id="customsLimitEur" class="w-full p-2 rounded-lg bg-white/20 mb-2" value="200">
          <label class="text-white/70 text-sm">Пошлина на превышение (%)</label>
          <input type="number" id="customsDutyPercent" class="w-full p-2 rounded-lg bg-white/20 mb-3" value="15">
          <button id="saveCustomsLimitsBtn" class="w-full bg-cyan-500 py-2 rounded-lg">Сохранить лимиты</button>
        </div>
    ...
    return `<p class="text-red-400">Ошибка загрузки FAQ: ${err.message}</p>`;
        ...
        <!-- 📢 Триггерные рассылки -->
        <div class="bg-white/10 backdrop-blur-md rounded-2xl p-4 mt-4 mb-4">
          <h3 class="text-white font-bold mb-3">📢 Триггерные рассылки (Push)</h3>
          <select id="promoAudience" class="w-full p-2 rounded-lg bg-white/20 mb-2 text-sm text-white">
            <option value="all">Все пользователи</option>
            <option value="vip">Только VIP (от 3 рефералов)</option>
            <option value="abandoned_cart">Брошенная корзина (нет заказов > 14 дней)</option>
            <option value="newbies">Новички (нет заказов)</option>
          </select>
          <textarea id="promoMessageText" class="w-full p-3 rounded-lg bg-white/20 border border-white/30 text-sm mb-3 text-white" placeholder="Текст сообщения... (например: Только 48 часов скидка на доставку!)" rows="3"></textarea>
          <button id="sendPromoBroadcastBtn" class="w-full bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 py-2 rounded-lg font-bold shadow-lg text-white">Отправить рассылку</button>
        </div>
          ...
          <div id="analyticsTab-analytics" class="hidden">
            <div class="flex gap-2 mb-4">
              <select id="analyticsPeriod" class="flex-1 p-2 rounded-lg bg-white/20 border border-white/30 text-xs">
                ...
                <option value="30" selected>30 дней</option>
                <option value="90">90 дней</option>
                <option value="9999">За всё время</option>
              </select>
              <button id="loadAnalyticsBtn" class="bg-cyan-500 hover:bg-cyan-600 px-4 py-2 rounded-lg text-xs font-bold shadow-lg">🔄 Загрузить</button>
            </div>
            
            <!-- Продвинутые метрики -->
            <div id="advancedMetricsBlock" class="grid grid-cols-2 gap-2 mb-4">
              <div class="bg-gradient-to-br from-purple-500/20 to-pink-500/20 rounded-xl p-3 border border-purple-500/30">
                <p class="text-white/60 text-[10px] uppercase tracking-wider mb-1">LTV (Ценность)</p>
                <p id="metricLTV" class="text-white text-xl font-bold shadow-sm">⏳</p>
              </div>
              <div class="bg-gradient-to-br from-green-500/20 to-emerald-500/20 rounded-xl p-3 border border-green-500/30">
                <p class="text-white/60 text-[10px] uppercase tracking-wider mb-1">Средний чек</p>
                <p id="metricAOV" class="text-white text-xl font-bold shadow-sm">⏳</p>
              </div>
              <div class="bg-gradient-to-br from-blue-500/20 to-cyan-500/20 rounded-xl p-3 border border-blue-500/30 col-span-2 flex justify-between items-center">
                <div>
                  <p class="text-white/60 text-[10px] uppercase tracking-wider mb-1">Воронка продаж (Конверсия)</p>
                  <p id="metricConversion" class="text-white text-xl font-bold shadow-sm">⏳</p>
                </div>
                <div class="text-right">
                  <p class="text-white/60 text-[10px] uppercase tracking-wider mb-1">Покупатели / Все</p>
                  <p id="metricBuyers" class="text-white text-sm font-bold">⏳</p>
                </div>
              </div>
            </div>
...
function attachAdminSuppliersHandlers() {
  ...
  document.querySelectorAll('.deleteSupplierBtn').forEach(btn => {
    ...
    async function renderCurrentScreen() {
      ...
      if (currentSubScreen === 'catalogHub') {
        contentDiv.innerHTML = shadowBannerHtml + await renderCatalogHub();
        ensureBackButtonForSubscreen();
        if (isShadowMode) attachShadowModeExit();
        return;
      }
...
function attachMarketplacesHandlers() {
  ...
  async function renderFilteredGrid() {
    ...
    grid.innerHTML = filtered.map(mp => {
      ...
      if (mp.requires_vpn || name.includes('poizon')) tagsHtml += '<span class="text-[10px] px-1.5 py-0.5 rounded-sm bg-red-500/20 text-red-400 border border-red-500/30 mt-1">Нужен VPN</span>';
      if (name.includes('1688')) tagsHtml += '<span class="text-[10px] px-1.5 py-0.5 rounded-sm bg-orange-500/20 text-orange-400 border border-orange-500/30 mt-1">Только опт</span>';
      if (name.includes('taobao') || name.includes('wechat')) tagsHtml += '<span class="text-[10px] px-1.5 py-0.5 rounded-sm bg-yellow-500/20 text-yellow-400 border border-yellow-500/30 mt-1">Сложная рег.</span>';
      ...
      <div class="product-card flex flex-col h-full">
        ...
        <div class="p-3 flex-1 flex flex-col relative">
          ...
          <p class="text-white/70 text-xs mb-1">${mp.country}</p>
          ...
          <div class="flex items-center justify-between mt-3 pt-2 border-t border-white/10">
            ...
            <button class="instruction-btn flex items-center gap-1 bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-400 px-3 py-1.5 rounded-lg text-xs font-bold transition" 
              ...
              <span class="ix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4M12 8h.01"/></svg></span> Как покупать
            </button>
          </div>
        </div>
      </div>
...
async function showFamilySettingsModal() {
  ...
  const loadAnalytics = async () => {
    ...
    try {
      const { data: orders, error } = await supabaseClient
        .from('orders').select('id, user_id, created_at, status, price_byn, prepayment_amount, items, source_url')
        .gte('created_at', since).order('created_at', { ascending: true });
      if (error) throw error;
      const os = orders || [];
      
      const { data: usersCountData, error: usersErr } = await supabaseClient
        .from('users').select('user_id', { count: 'exact' });
      const totalUsers = usersCountData ? usersCountData.length : 1; // защита от деления на 0

      // Sales by day & Advanced Metrics
      const byDay = {};
      let totalRevenue = 0;
      let totalRevenueDelivered = 0;
      const uniqueBuyers = new Set();
      const uniqueBuyersDelivered = new Set();

      os.forEach(o => {
        const day = (o.created_at||'').slice(0, 10);
        const rev = o.price_byn || o.prepayment_amount || 0;
        ...
        totalRevenue += rev;
        if (o.user_id) uniqueBuyers.add(o.user_id);
        
        if (o.status === 'delivered') {
          totalRevenueDelivered += rev;
          if (o.user_id) uniqueBuyersDelivered.add(o.user_id);
        }
      });
      const days = Object.keys(byDay).sort();
      
      // Update Advanced Metrics UI
      const aov = os.length > 0 ? (totalRevenue / os.length) : 0;
      const ltv = uniqueBuyersDelivered.size > 0 ? (totalRevenueDelivered / uniqueBuyersDelivered.size) : 0;
      const conversion = totalUsers > 0 ? ((uniqueBuyers.size / totalUsers) * 100) : 0;
      ...
      const metricBuyers = document.getElementById('metricBuyers');
      
      if (metricLTV) metricLTV.innerText = ltv.toFixed(2) + ' BYN';
      if (metricAOV) metricAOV.innerText = aov.toFixed(2) + ' BYN';
      if (metricConversion) metricConversion.innerText = conversion.toFixed(1) + '%';
      if (metricBuyers) metricBuyers.innerText = uniqueBuyers.size + ' / ' + totalUsers;
      return `
        <div class="space-y-4">
  ...
  chatInterval = setInterval(fetchAndRenderMessages, 4000);
      ...
      if (currentSubScreen === 'about') {
        contentDiv.innerHTML = `
          <button class="global-back-btn" id="aboutBackBtn">← Назад</button>
          <div class="bg-white/10 backdrop-blur-md rounded-2xl p-5 shadow-lg relative mt-4">
            <div class="text-center mb-6">
              <img src="https://i.ibb.co/9kJTjpjT/1-Logo.png" alt="ICE LOGIX" class="h-20 mx-auto mb-4">
              <h2 class="text-2xl font-bold text-white">ICE LOGIX</h2>
              <p class="text-cyan-400 font-semibold text-sm">Ваш надежный байер-сервис</p>
            </div>
            
            <div class="space-y-4 text-white/80 text-sm">
              <p>📍 <b>Мы находимся:</b> Беларусь, Несвиж.</p>
              <p>🛍 <b>Что мы делаем:</b> Выкупаем и возим товары с любых площадок и от поставщиков, которые не работают напрямую с РБ. Одежда, обувь, аксессуары из Европы, Китая, Японии, Южной Кореи, США, ОАЭ, России, Турции и Вьетнама.</p>
              <div class="bg-blue-500/10 p-3 rounded-lg border border-blue-500/20">
                <p class="text-blue-300 text-xs">⚠️ Обратите внимание: в первые месяцы работы доступен выкуп только из Европы, России и Китая. Остальные направления будут доступны чуть позже!</p>
              </div>
              <p>🤝 <b>Наш партнер:</b> Работаем совместно с ShopbyShop. У нас есть склады в России, Польше и Китае. На данный момент единая ставка из Китая — 10$ за кг.</p>
              <p>📦 <b>Планы:</b> В будущем мы планируем доставлять абсолютно любые товары, разрешенные для ввоза в РБ, но пока концентрируемся на одежде и обуви.</p>
              <p>💼 <b>Юридическая чистота:</b> Мы работаем полностью легально, платим налоги с нашей комиссии (20%) и заключаем онлайн-договор с каждым клиентом при оформлении заказа.</p>
            </div>
          </div>
          ${renderFooter()}
        `;
        const aboutBackBtn = document.getElementById('aboutBackBtn');
        if (aboutBackBtn) aboutBackBtn.onclick = () => { currentSubScreen = null; renderCurrentScreen(); };
        return;
      }
...
window.showVipSectionModal = () => {
  ...
  const vipItems = [
    ...
    // ==================== РЕНДЕР КАТАЛОГОВ (ВЫБОР) ====================
    async function attachSubScreenHandlers() {
      const aboutBackBtn = document.getElementById('aboutBackBtn');
      if (aboutBackBtn) aboutBackBtn.onclick = () => { currentSubScreen = null; renderCurrentScreen(); };
    }
...
async function renderLegitCheck() {
  ...
  if (!disclaimerAccepted) {
    return `
      ...
      <div class="glass-card page-enter font-sans">
  ...
  // Load user's submitted legit check requests
  ...
  try {
    ...
    if (checks && checks.length > 0) {
      requestsListHtml = `
        <div class="glass-card mb-4 page-enter font-sans">
...
window.publishToResale = async (orderId, orderTitle) => {
  ...
  tgUtil.showPopup({
    title: 'ICE Resale',
    message: 'Введите цену, за которую вы хотите продать этот товар (в BYN):\nКомиссия платформы 5% будет удержана при успешной сделке.',
    buttons: [{ id: 'ok', type: 'ok' }, { type: 'cancel' }]
  }, async (btnId) => {
    // В реальном Telegram Web App нет prompt() внутри popup с вводом текста, 
    // поэтому мы будем использовать системный prompt (fallback).
    // Либо можно открыть модалку.
  });
  ...
  try {
    ...
    let html = `
      <div class="flex items-center gap-3 mb-6">
        <button class="w-10 h-10 rounded-full flex items-center justify-center bg-white/5 border border-white/10 hover:bg-white/10 transition-colors" onclick="switchTab('home')">
        ...
        <h2 class="text-xl font-black text-white uppercase tracking-wider">ICE Resale <span class="text-pink-400">Пристрой</span></h2>
      </div>
      <p class="text-xs text-white/50 mb-4 bg-white/5 p-3 rounded-xl">Покупайте новые вещи напрямую у других клиентов, которым не подошел размер. Вещи уже в Беларуси!</p>
    `;
    ...
    if (!data || data.length === 0) {
      ...
      html += data.map(item => `
        <div class="glass-card flex flex-col relative overflow-hidden">
          <div class="absolute top-2 right-2 bg-pink-500/80 text-white text-[10px] font-bold px-1.5 py-0.5 rounded border border-pink-400 backdrop-blur-md">В наличии</div>
          <div class="p-3 flex-1">
            ...
            <p class="text-cyan-400 font-bold mb-2">${item.price_byn} BYN</p>
            <div class="flex items-center gap-1 mt-auto text-[10px] text-white/50">
  ...
  try {
    ...
    if (ctxStatus && window.Chart) {
      new Chart(ctxStatus, {
        type: 'pie',
        data: {
          labels: Object.keys(statusCounts).map(s => (window.getStatusText ? window.getStatusText(s) : s).replace(/[^\wа-яА-Я\s()]/gi, '').trim()),
          datasets: [{
