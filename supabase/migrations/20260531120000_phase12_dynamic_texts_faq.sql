-- Phase 12: dynamic_texts (editable interface/bot texts) + faq_items upgrade (is_published)
...
DROP POLICY IF EXISTS "Enable all actions for dynamic_texts" ON dynamic_texts;
CREATE POLICY "Enable all actions for dynamic_texts" ON dynamic_texts FOR ALL USING (true) WITH CHECK (true);

-- ==== faq_items (knowledge base) — table may already exist, ensure shape ====
...
ALTER TABLE faq_items ADD COLUMN IF NOT EXISTS is_published boolean NOT NULL DEFAULT true;
ALTER TABLE faq_items ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Enable all actions for faq_items" ON faq_items;
CREATE POLICY "Enable all actions for faq_items" ON faq_items FOR ALL USING (true) WITH CHECK (true);

-- ==== Seed dynamic_texts (idempotent by key) ====
...
-- ==== Seed faq_items ТОЛЬКО если таблица пуста (не дублируем существующие записи) ====
INSERT INTO faq_items (category, question, answer, order_index, is_published)
SELECT v.category, v.question, v.answer, v.order_index, true
FROM (VALUES
...
) AS v(category, question, answer, order_index)
